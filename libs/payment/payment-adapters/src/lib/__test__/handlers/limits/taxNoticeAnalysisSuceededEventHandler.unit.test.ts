import { TaxNoticeAnalysisProps, TaxNoticeAnalysisSucceeded } from '@oney/profile-messages';
import {
  AskUncapping,
  AskUncappingCommand,
...
describe('Unit test of ProfileActivatedEventHandler', () => {
  ...
  beforeAll(async () => {
    nock.back.fixtures = path.resolve(`${__dirname}/fixtures/taxAnalysis`);
  ...
  it('should handle call the use case with the expected command', async () => {
    ...
    const props: TaxNoticeAnalysisProps = {
      globalGrossIncome: 'test',
      personalSituationCode: 'test',
    };
    const metadata = {
    ...
    const event = new TaxNoticeAnalysisSucceeded(props, metadata);

    // mock useCase
    const spy = (AskUncapping.prototype.execute = jest.fn());

    const handlersUseCase = kernel.get(AskUncapping);
    const handler = new TaxNoticeAnalysisSucceededEventHandler(handlersUseCase);
