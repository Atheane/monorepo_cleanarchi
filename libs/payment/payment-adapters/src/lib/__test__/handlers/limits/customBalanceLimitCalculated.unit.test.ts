...
describe('Unit test of ProfileActivatedEventHandler', () => {
  ...
  beforeAll(async () => {
    nock.back.fixtures = path.resolve(`${__dirname}/fixtures/customLimit`);
  ...
  it('should handle call the use case with the expected command', async () => {
    ...
    // const { nockDone } = await nockBack('updateBalanceLimit.json', { before });
    ...
    const spy = (Uncap.prototype.execute = jest.fn());

    const handlersUseCase = kernel.get(Uncap);
    const handler = new CustomBalanceLimitCalculatedEventHandler(handlersUseCase, defaultLogger);

    await handler.handle(event);

    const expectedCommand: UncapCommand = {
      uid: props.uId,
      monthlyGlobalOutAllowance: props.customBalanceLimit,
      eligibility: props.customBalanceLimitEligibility,
