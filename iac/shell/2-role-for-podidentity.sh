...
export IDENTITY_RESOURCE_GROUP=$2
...
echo "Assigning 'Network contributor for ingress' role to ${ID}"
az role assignment create --role "Contributor" --assignee "${ID}" --scope "/subscriptions/${SUBSCRIPTION_ID}/resourcegroups/${NODE_RESOURCE_GROUP}"
