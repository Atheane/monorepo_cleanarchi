...
import { BillSubscriptions } from '@oney/subscription-core';
...
const scheduleBilling = async function (context: Context): Promise<void> {
  ...
  const scheduleBilling = container.get(BillSubscriptions);
  await scheduleBilling.execute();
  context.res = {
    status: 'DONE',
  };
};
