...
import { IAppConfiguration, SplitPaymentSchedule, SplitPaymentScheduleError } from '@oney/credit-core';
...
describe('Get user split contracts unit testing - implementation inMemory', () => {
  ...
  it('should return an empty array when no user as no contract', async () => {
    ...
    const result = await dependencies.getSplitContracts.execute({ uid: userId });
  ...
  it('should return true when token azure with permissions.read=all', async () => {
    ...
    const isAuthorize = await dependencies.getSplitContracts.canExecute(identity, { uid: userId });
