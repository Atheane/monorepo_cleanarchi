...
import { ProfessionalCategoriesList } from '@oney/profile-core';
...
describe('GetProfessionalActivitiesList integration api testing', () => {
  ...
  it('Should return error when is user is not authenticated', async () => {
    await request(app)
      .get('/profile/resources/onboarding/step/fiscalstatus/professionalactivitieslist')
      .send({})
      .expect(401);
  });
  ...
  it('Should return the fiscal countries list', async () => {
    ...
    const facematchUser = await profileDb.generate('okGetProfessionalActivitiesList', ProfileStatus.ON_HOLD);
    ...
    await request(app)
      ...
      .expect(200)
      .expect(res => {
        expect(res.body).toEqual(ProfessionalCategoriesList);
      });
  });
});
